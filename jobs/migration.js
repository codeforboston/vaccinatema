...
async function pullClinics() {
    const records = await base('MaVaccSites_Today').select().all();
...
function convertAirtableObjectToSqlObject(airTableObject) {
    return {
        ...
        bookinglink: airTableObject['Book an appointment']
            ? airTableObject['Book an appointment']
            : '',
        address: airTableObject['Full Address'],
        serves: airTableObject['Serves'],
        vaccinesoffered: null,
        siteinstructions: airTableObject['Instructions at site']
            ? airTableObject['Instructions at site']
            : '',
        ...
        accessibility: airTableObject['Accessibility']
            ? airTableObject['Accessibility'].replace('\n', '')
            : '', // to add
