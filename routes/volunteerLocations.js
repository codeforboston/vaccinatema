...
router.post('/',
    ...
    async function(req, res) {
        ...
        try {
            const volunteer = await volunteersDb.createVolunteerMapping(req.body.volunteerId, req.body.locationId);
...
router.get('/',
    query('volunteerId').isNumeric().withMessage('Numeric volunteer id is required'),
    async function(req, res) {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
...
router.delete('/',
    query('volunteerId').isNumeric().withMessage('Numeric volunteer id is required'),
    query('locationId').isNumeric().withMessage('Numeric location id is required'),
    async function(req, res) {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({ errors: errors.array() });
