...
router.get(
    ...
    [
        query('latitude')
            .optional()
            .isNumeric()
            .withMessage('Only numbers allowed for lat/long'),
        query('longitude')
            .optional()
            .isNumeric()
            .withMessage('Only numbers allowed for lat/long'),
        query('zipcode')
            .optional()
            .isNumeric()
            .withMessage('Only numbers allowed for zipcode'),
        query('rangeMiles')
            .optional()
            .isNumeric()
            .withMessage('Only numbers allowed for rangeMiles'),
    ...
    async function (req, res) {
        try {
            ...
            if (locationName) {
                ...
                let locations = await locationsDb.getLocationsByName(
                    locationName
                );
                res.json(locations);
            } // if lat/long provided use that to do a geolookup
            else if (homeLatitude && homeLongitude) {
                let locations = await locationsDb.getLocationsCloseToGeo(
                    homeLatitude,
                    homeLongitude,
                    homeRangeInMiles
                );
...
router.post(
    ...
    async function (req, res) {
        ...
        try {
            ...
            let createdLocation = await locationsDb.createLocation(
                locationToCreate
            );
...
router.put(
    ...
    async function (req, res) {
        ...
        try {
            ...
            let updatedLocation = await locationsDb.updateLocation(
            ...
            console.log(
                `ERROR OCCURRED UPDATING LOCATION! locationId: ${req.params.locationId} body: ${req.body} error: ${error}`
            );
...
router.put(
    ...
    async function (req, res) {
        ...
        try {
            let updatedLocation = await locationsDb.insertLocationAvailability(
                req.params.locationId,
                req.body
            );
            res.status(200).json(updatedLocation);
            ...
            console.log(
                `ERROR OCCURRED ADDING LOCATION AVAILABILITY! locationId: ${req.params.locationId} body: ${req.body} error: ${error}`
            );
...
router.delete(
    '/:locationId/availability/:locationAvailabilityId',
    param('locationId').isNumeric(),
    param('locationAvailabilityId').isNumeric(),
    async function (req, res) {
        try {
            await locationsDb.deleteLocationAvailability(
                req.params.locationId,
                req.params.locationAvailabilityId
            );
            res.status(200).json('deleted');
        } catch (error) {
            console.log(
                `ERROR OCCURRED DELETING LOCATION AVAILABILITY! locationId: ${req.params.locationId} locationAvailabilityId: ${req.params.locationAvailabilityId} error: ${error}`
            );
...
router.delete(
    ...
    async function (req, res) {
        try {
            const result = await locationsDb.deleteLocation(
                req.params.locationId
            );
            res.status(200).json(result);
        } catch (error) {
            console.log(
                `ERROR OCCURRED DELETING LOCATION! locationId: ${req.params.locationId} error: ${error}`
            );
            let errorObj = {error};
            res.status(500).send(errorObj);
        }
