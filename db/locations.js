...
const getLocationsByName = async (locationName) => {
    try {
        ...
        console.error(
            'an error occurred fetching locations for location name:  ' +
                locationName,
            error
        );
        throw new Error(
            'an unexpected error occurred reading from locations DB for locationName'
        );
    }
};
...
const getLocationsCloseToGeo = async (homeLat, homeLong, rangeInMiles) => {
    try {
        ...
        console.log(
            `LOCATION LOOKUP BASED ON GEO: ${homeLat} ${homeLong} RangeInMeters: ${rangeInMeters}`
        );
...
const createLocation = async (newLocation) => {
    try {
        ...
        const {rows} = await pool.query(query, [
            newLocation.name,
            newLocation.bookinglink,
            newLocation.serves,
            newLocation.siteinstructions,
            newLocation.daysopen,
            newLocation.county,
            newLocation.latitude,
            newLocation.longitude,
            newLocation.address,
            newLocation.vaccinesoffered,
        ]);
        ...
        if (newLocation.initialAvailability) {
            let availability = await insertLocationAvailability(
                insertedRow.id,
                newLocation.initialAvailability
            );
        ...
        console.error(
            `an error occurred creating location ${newLocation}`,
            error
        );
        throw new Error(
            `An unexpected error occurred saving location with error: ${error}`
        );
    }
};
...
const updateLocation = async (locationId, locationObject) => {
    try {
        ...
        console.error(
            `an error occurred updating location ${locationObject}`,
            error
        );
...
const insertLocationAvailability = async (locationId, availabilitydetails) => {
    try {
        ...
        console.log(
            `ADDING LOC AVAILABILITY for locationId: ${locationId} and availabiltyDetails ${JSON.stringify(
                availabilitydetails
            )} `
        );
        ...
        console.error(
            `an error occurred upserting location availability ${locationId}`,
            error
        );
        throw new Error(
            `An unexpected error occurred saving location availability error: ${error}`
        );
    }
};
...
const deleteLocation = async (locationId) => {
    try {
        ...
        console.error(
            `an error occurred deleting location ${locationId}`,
            error
        );
 ...
 */
const deleteLocationAvailability = async (
    locationId,
    locationAvailabilityId
) => {
    try {
        console.log(
            `DELETED LOCATION AVAIL locId: ${locationId} locAvailId: ${locationAvailabilityId}`
        );
        let query =
            'DELETE FROM location_availability where id = $1 and location_id = $2';
        await pool.query(query, [locationAvailabilityId, locationId]);
    } catch (error) {
        console.error(
            `an error occurred deleting location avail ${locationAvailabilityId}`,
            error
        );
